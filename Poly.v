...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [| h t Hl].
  -simpl. reflexivity.
  -simpl. rewrite -> Hl. reflexivity.
Qed.
...
Proof.
  intros X l1 l2. induction l1 as [|h t Hl1].
  -simpl. reflexivity.
...
(** **** Exercise: 3 stars, standard (map_rev) 
    ...
    auxiliary lemma. *)
Lemma map_add_distr: forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
  map f (l1 ++ l2 ) = map f l1 ++ map f l2.
Proof. 
  intros X Y f l1 l2. induction l1 as [|h t Hl1].
  -simpl. reflexivity.
  -simpl. rewrite <- Hl1. reflexivity.
Qed.
...
Proof.
  intros X Y f l. induction l as [|h t Hl].
  -simpl. reflexivity.
  -simpl. rewrite <- Hl. rewrite -> map_add_distr. reflexivity. 
Qed.
(** [] *)
...
Proof.
  intros X l. induction l as [|h t Hl].
  -simpl. reflexivity. 
  (* 这里的simpl.是不彻底的（函数定义没有完全展开），但使用reflexivity又能够完成证明，有点迷 *)
  (* 似乎simpl对高阶函数的展开是不彻底的，而reflexivity则展开的更多一些*)
  -simpl. rewrite <- Hl. reflexivity.
Qed.
(** [] *)
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y := fold (fun x (y : list Y) => (f x) :: y) l nil. 
