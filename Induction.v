...
Proof.
  ...
  -simpl. rewrite -> Hn. rewrite -> plus_n_Sm. reflexivity.
Qed.
...
Proof.
  ...
  -simpl. reflexivity.
  -rewrite -> Hn. simpl.
  assert(H2: forall m : bool, negb ( negb m) = m).
  --intros m. destruct m.
  ---simpl. reflexivity.
  ---simpl. reflexivity.
...
(** **** Exercise: 1 star, standard, optional (destruct_induction) 
    ...
    and [induction].
    destruct仅对一个变量的值做了分类讨论生成了子目标，最终证明结果不依靠数学归纳法
    （即没有归纳假设），而induction除了分类生成子目标还生成了归纳假设（假设n成立），
    其最终证明的正确性由数学归纳法的正确性保证。
...
(** **** Exercise: 2 stars, advanced, especially useful (plus_comm_informal) 
    ...
    Theorem: Addition is commutative. i.e. For any n, m,
        n + m = m + n.
    Proof: 
        对n进行归纳分析.
        > 首先假设 n = 0. 则原命题转化为
        ...
        m = m + 0.
        根据定理plus_n_O，原命题得证
        > 其次假设 n = S n', 并且 n' + m = m + n'
        原命题转化为 S n' + m = m + S n'
        根据加法的定义，原命题转化为S (n' + m) = m + S n'
        根据归纳假设，原命题转化为S (m + n') = m + S n'
        根据定义plus_n_O，原命题得证
    Qed.
*)
...
Theorem mult_n_O : forall n, n * 0 = 0.
Proof.
  intros n. induction n as [|n' Hn].
  -reflexivity.
  -simpl. rewrite -> Hn. reflexivity.
...
Proof.
  ...
  -simpl. rewrite -> mult_n_Sm. rewrite -> Hm. reflexivity.
Qed.
...
Proof.
  intros n m p. intros H. induction p as [|p' Hp].
...
Proof.
  intros n m p. induction n as [|n' Hn].
  -simpl. reflexivity.
  -simpl. rewrite -> Hn. rewrite <- plus_assoc. reflexivity.
Qed. 
...
Proof.
  intros n m p. induction n as [|n' Hn].
  -simpl. reflexivity.
  -simpl. rewrite -> Hn. rewrite -> mult_plus_distr_r. reflexivity.
Qed.
